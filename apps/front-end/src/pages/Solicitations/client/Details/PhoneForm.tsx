import { Button, IconPencil, IconSave, Input, Panel, Text } from "@app/ui";
      ...
      {!editMode ? (
        ...
        <Panel className="w-full border border-primary">
          <form onSubmit={handleSubmit(handleSave)}>
            ...
            <div className="flex flex-col mt-6 gap-1 mb-5">
              <Text className="font-extrabold text-lg" as="h1">Marca - <select {...register(phoneQuestions[0].questionId)} className="form-select rounded bg-black form-select-lg text-white">{phoneQuestions[0].options.map((item) => {
                return <option value={item.optionId}>{item.text}</option>
              })}</select></Text>

              {brand === 'apple' && (<Text className="font-extrabold text-lg" as="h1">Modelo - <select {...register(phoneQuestions[1].questionId)} className="form-select rounded bg-black form-select-lg text-white">{appleModels.map((item) => {
              ...
              <Text className="font-extrabold text-lg" as="h1">{phoneQuestions[2].question} - <select {...register(phoneQuestions[2].questionId)} className="form-select rounded bg-black form-select-lg text-white">{phoneQuestions[2].options.map((item) => {
                return <option value={item.optionId}>{item.text}</option>
              })}</select></Text>
              <Text className="font-extrabold text-lg" as="h1">{phoneQuestions[3].question} - <select {...register(phoneQuestions[3].questionId)} className="form-select rounded bg-black form-select-lg text-white">{phoneQuestions[3].options.map((item) => {
                return <option value={item.optionId}>{item.text}</option>
              })}</select></Text>
              <Text className="font-extrabold text-lg" as="h1">{phoneQuestions[4].question} - <select {...register(phoneQuestions[4].questionId)} className="form-select rounded bg-black form-select-lg text-white">{phoneQuestions[4].options.map((item) => {
                return <option value={item.optionId}>{item.text}</option>
              })}</select></Text>
            </div>
          </form>
